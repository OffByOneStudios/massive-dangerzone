...
Madz is designed for massive projects which large and diverse sets of requriments. Here are some cases madz will excel at:
...
* Abstracting platform features behind plugins automatically. Plugins for other features will simply be ignored when run on those systems. Features can include operation systems, hardware and software installed, processor features, and anything else.
* Abstrcting interpreter and compiler choice. Language configuration is seperate from compiler choices. Specify custom compilers and interpreters and compare their performence, or choose a stock one.
* Organizing large projects. Organize a project however you see fit with many folder and documentation structures. Overlay your additional plugins on top of an exisiting plugin system by simply adding a directory to the load list and writing a custom start script.
