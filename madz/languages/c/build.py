...
class Builder(object):
    ...
    def build(self):
        ...
        subprocess.Popen(["gcc", "-c", "-I"+self._w_dir, "-fpic"] + compile_files, cwd=self._b_dir)

        object_files = map(lambda c: os.path.basename(c)[:-2] + ".o", compile_files)

        subprocess.Popen(["gcc", "-shared", "-o", os.path.join(self._o_dir, self.plugin_stub.id.namespace + ".madz")] + object_files, cwd=self._b_dir)
