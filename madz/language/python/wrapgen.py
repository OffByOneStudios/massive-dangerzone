...
"""
        ...
        fragments = {"function_hooks":"", "madzpath" : madz_path.path}
        for node in self.description.definitions():
            if isinstance(node.type.get_type(), pdl.TypeFunction):
...
"""
    ...
    def generate(self):
        ...
        code_fragments = {
            "autogenerated_module_path": os.path.normcase(self.language.wrap_directory.path)),
            ...
            "init_path": os.path.normcase(self.language.get_plugin_init().path),
            "ctypes_wrapper_path": os.path.normcase(self.language.get_python_ctypes_wrapper().path),
        ...
        c_source = py_gen.make_c_init(repr(self.language.get_python_code_filename().path)[1 : -1])
