...
class PythonGenerator(object):
    ...
    def __init__(self, dependencies, namespace, description, executeable=False):
...
"""
    ...
    def make_madz_execute(self):
        """Generate gluecode for executable plugins"""
        res = \
"""
    # Hook Up execute Function
    EXECUTE_FN_TYPE = CFUNCTYPE(void)
    shared_object.___madz_execute = EXECUTE_FN_TYPE(user_code_module.__execute__)
"""
        return res
    # C Header Generation
...
"""
        ...
        fragments ={"fn_dec" : "", "function_pointers" : "", "madz_execute": "void ___madz_execute();" if self.executable else "",}
...
"""
        ...
        fragments = {"function_hooks":"", "madzpath" : str(madz_path)}
        for node in self.description.definitions():
            if isinstance(node.type.get_type(), pdl.TypeFunction):
...
The Actual class forms the middle layer of a ctypes, madz_types, python_types abstraction layer.
"""
...
class InternalMadzMeta(type):
    ...
    class MadzOverrideDescriptor(object):
        ...
        def __init__(self, name, c_type, override=None):
            """Default Constructor
            ...
            self._c_type = c_type
            if issubclass(c_type[0], _ctypes.CFuncPtr) and \
                len(c_type[0]._argtypes_) > 0 and \
                issubclass(c_type[0]._argtypes_[0], _ctypes._Pointer):
                self._ismethod = True
            else:
        ...
        def __get__(self, instance, owner):
            if instance is None:
                ...
                if self._override is None:
                    #TODO cast below to madz ctypes object
                    raw_res = getattr(instance.__madz_object__, self._name)
                    res = raw_res
                    type = internal_madz_type(self._c_type[0])
                    if type is None:
                        return res
                    else:
                        return type(res)
        ...
        def __set__(self, instance, value):
            if instance is None:
                ...
                #TODO check if value is ctypes madztypes or pythonz
                if isinstance(value, ctypes._SimpleCData) or isinstance(value, _ctypes._Pointer):
                    # A Ctype
                    pass
                
                elif isinstance(value, _ctypes.CFuncPtr):
                    # A callable Ctype
                    pass
                    
                elif isinstance(type(type(value)), InternalMadzMeta):
                    # A madz wrapper type
                    ...
                    # A Python object.
                    if callable(value):
                        # If method, check/drop first arg:
                        if self._ismethod:
                            auto_unthis_value = value
                            def auto_unthis(this, *args, **kwargs):
                                #TODO: Check self == this
                                return auto_unthis_value(*args, **kwargs)
                            value = auto_unthis
                        # Assume they are attempting to assign a function with a valid sig for this slot
                        value = self._c_type[0](value)
                        
                #isinstance(px, ctypes._SimpleCData)
                setattr(instance.__madz_object__, self._name, value)
            
            
    def __new__(meta, name, parents, attrs):
        InternalMadzMeta.override_attrs(InternalMadzMeta.get_attr("__madz_ctype__", parents, attrs), attrs)
    ...
    def override_attrs(c_type, attrs):
        ...
        for attr_name, attr_val in list(attrs.items()):
            ...
            if attr_name in field_dict:                    
                ...
                attrs[attr_name] = InternalMadzMeta.MadzOverrideDescriptor(name=attr_name, c_type=field, override=attr_val)
                del field_dict[attr_name]
        
        for field_name, field_val in field_dict.items():
            attrs[field_name] = InternalMadzMeta.MadzOverrideDescriptor(name=field_name, c_type=field_val)
...
def internal_madz_type(c_type):
    ...
    if not (inspect.isclass(c_type) and (issubclass(c_type, ctypes.Structure) or issubclass(c_type, _ctypes._Pointer))):
        return None
    ...
    if not _HashMe(c_type) in _internal_madz_type_cache:
        class Actual(metaclass=InternalMadzMeta):
            ...
            __madz_ctype__ = c_type._type_ if hasattr(c_type, "_type_") else c_type
...
class cstdlib(object):  
    _dll = ctypes.cdll.LoadLibrary(_ctypes_find_library('MSVCRT'))
    malloc = _dll.malloc
...
"""
    ...
    def generate(self):
        ...
        code_fragments = {
            "autogenerated_module_path": os.path.normcase(str(self.language.wrap_directory)),
            "autogenerated_module": str(self.language.get_python_autogenerated_module()),
            "new_autogenerated_module": str(self.language.get_python_new_autogenerated_module()),
            "fix_plugin": py_gen.make_fix_plugin(),
            "module_namespace": "_madz__{}".format(str(self.plugin_stub.id.namespace).replace(".", "__")),
            "init_path": os.path.normcase(str(self.language.get_plugin_init())),
            "ctypes_wrapper_path": os.path.normcase(str(self.language.get_python_ctypes_wrapper())),
            ...
            "plugin_cname": str(self.language.output_directory.file("{}.madz".format(self.plugin_stub.id.namespace))),
            "function_callbacks": py_gen.make_function_callbacks(),
            "function_stubs": py_gen.make_function_stubs(),
            "madz_execute" : py_gen.make_madz_execute() if self.plugin_stub.executable else "",
            }
        ...
        c_source = py_gen.make_c_init(repr(str(self.language.get_python_code_filename()))[1 : -1])
        ...
        with self.language.get_python_autogenerated_module().open("w") as f:
            f.write(module_string)

        with self.language.get_python_new_autogenerated_module().open("w") as f:
            f.write(module_string)

        with self.language.get_python_ctypes_wrapper().open("w") as f:
            f.write(self.ctypes_wrapper_template)

        with self.language.get_c_code_filename().open("a") as f:
            f.write("\n{}\n".format(c_source))

        with self.language.get_python_code_filename().open("w") as f:
